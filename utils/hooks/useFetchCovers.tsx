...
import { useIpfs } from "lib/ipfs/context";
import { calculateCoverStats, defaultStats } from "@utils/helpers/cover";
...
export const useFetchCovers = () => {
  ...
  useEffect(() => {
    ...
    function exec() {
      ...
      const _data = _covers.map((_cover: any) => {
  ...
  const getInfoByKey = (coverKey: any) => {
    return data.find((x: any) => x.key === coverKey) || { stats: defaultStats };
  };
